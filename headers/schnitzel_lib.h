#pragma once
#include <stdio.h>
// for malloc function (BumpAllocator)
#include <stdlib.h>
// To get memset
#include <string.h>
#include <sys/stat.h>
...
#elif __linux__
#define DEBUG_BREAK() __builtin_debug_trap();
#elif __APPLE__
...
BumpAllocator make_bump_allocator(size_t size)
{
	...
	{
		ba.capacity = size;
		memset(ba.memory, 0, size); // Set the memory to 0
		return ba;
	}
	else
	{
...
char* bump_alloc(BumpAllocator* bumpAllocator, size_t size) {
	char* result;
	size_t alignedSize = size + 7 & ~7; // make sure the first 4 bits are set to 0
